...
`bbscope` is a single Go 1.25 CLI (module `github.com/cozyGarage/bbscope/v2`). There is no long-running server; you build one binary and run subcommands. Standard commands live in the `Makefile`, `README.md`, and `docs/CONTRIBUTING.md` — use those; notes below only cover non-obvious caveats.
...
- Lint: `golangci-lint` is installed to `~/go/bin`, which is not on `PATH` by default — run `PATH=$PATH:~/go/bin golangci-lint run ./...` (or `make lint`).
...
- Storage integration tests in `pkg/storage/integration_test.go` are gated on the `TEST_DB_URL` env var and skip when it is unset. To run them against the local DB: `TEST_DB_URL="postgres://bbscope:devpass@127.0.0.1:5432/bbscope?sslmode=disable" go test ./pkg/storage/...`. They isolate themselves with a unique per-test platform name and clean up afterwards.
